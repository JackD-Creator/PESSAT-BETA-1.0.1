...
export async function getFeeds() {
  const { data, error } = await supabaseAdmin.from('feeds').select('*').order('name');
...
export async function getFeedInventory() {
  const { data, error } = await supabaseAdmin.from('feed_inventory').select('*, feeds(name, category)').order('feeds(name)');
  if (error) throw error;
...
export async function getFeedPurchases() {
  const { data, error } = await supabaseAdmin.from('feed_purchases').select('*, feeds(name)').order('purchase_date', { ascending: false }).limit(50);
  if (error) throw error;
...
export async function getFeedConsumption() {
  const { data, error } = await supabaseAdmin.from('feed_consumption').select('*, feeds(name)').order('consumption_date', { ascending: false }).limit(50);
  if (error) throw error;
...
export async function getFeedFormulas() {
  const { data, error } = await supabaseAdmin.from('feed_formulas').select('*').order('name');
  if (error) throw error;
  return data as FeedFormula[];
}

export async function getFeedFormulaItems(formulaId: string) {
  const { data, error } = await supabaseAdmin.from('feed_formula_items').select('*, feeds(name)').eq('formula_id', formulaId);
  if (error) throw error;
...
export async function getNutritionRequirements(species?: string) {
  let q = supabaseAdmin.from('nutrition_requirements').select('*');
...
export async function getFeedInventorySummary() {
  const { data, error } = await supabaseAdmin.from('feed_inventory').select('total_cost, min_threshold, quantity_on_hand');
  if (error) throw error;
  return {
